...
countries=$(cat $shell_source_dir/../$detection_dir/src-shell/countries.txt | cut -d" " -f1)

for country in $countries;do
  echo "Fetching $country"
  outdir=$shell_source_dir/../data/"$country"wiki-$version-results
  mkdir -p $outdir
  rsync "$user$dir/wikipedia-reverts-geolocating/data/"$country"wiki-$version-results/*.{txt,tex,pdf}" $outdir
